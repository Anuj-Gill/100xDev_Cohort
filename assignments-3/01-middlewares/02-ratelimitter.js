const request = require('supertest');
const assert = require('assert');
const express = require('express');
const app = express();
// You have been given an express server which has a few endpoints.
// Your task is to create a global middleware (app.use) which will
// rate limit the requests from a user to only 5 request per second
// If a user sends more than 5 requests in a single second, the server
// should block them with a 404.
// User will be sending in their user id in the header as 'user-id'
// You have been given a numberOfRequestsForUser object to start off with which
// clears every one second

let numberOfRequestsForUser = {};
setInterval(() => {
  for(a in numberOfRequestsForUser){
    numberOfRequestsForUser[a] = 0;
  };
},1000);

function rateLimitter(req, res, next) {
  let id = req.headers["user-id"];
  if (id in numberOfRequestsForUser) {
    let userCount = numberOfRequestsForUser[id];
    userCount++;
    if (userCount > 5) {
      res.status(404).json({ msg: "You are blocked!!" });
    } else {
      numberOfRequestsForUser[id] = userCount;
      next();
    }
  } else {
    res.json({ msg: "User does not exists!!" });
  }
}

app.use(rateLimitter);

app.get('/',(req,res) => {
  res.send("Working properly")
})

app.get('/user',rateLimitter, function(req, res) {
  if(count >= 5){
    res.status(404).json({msg: "You are blocked!!"})
  } 
  else {

    res.status(200).json({msg: "fine", numberOfRequestsForUser})
  }
});

app.post('/user', function(req, res) {
  let userId = Number(req.headers["user-id"]);
  if(userId in numberOfRequestsForUser){
    res.json({msg: "User already exists"})
  }
  else {
    numberOfRequestsForUser[userId] = count;
    res.status(200).json({ msg: `created dummy user ${userId}` });
  }
});

module.exports = app;